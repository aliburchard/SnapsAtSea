...
ggplot(data = filter(class_times, user_status %in% c("new", "old")), aes(duration, fill = workflow_name, colour = workflow_name)) + 
  geom_density(alpha = .2) + scale_x_log10() +
  facet_grid( device ~ user_status)

jpeg(filename = "Figures/duration_v_device.jpg")
